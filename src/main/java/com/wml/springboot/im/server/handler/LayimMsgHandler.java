...
import com.wml.springboot.im.common.util.TokenVerify;
import com.wml.springboot.im.packet.LayimContextUserInfo;
import com.wml.springboot.im.packet.LayimToClientOnlineStatusMsgBody;
...
public class LayimMsgHandler implements IWsMsgHandler {
    ...
    private ImService getImService(){
        return (ImService) SpringUtil.getBean("imService");
    }
    /**
    ...
    private HttpResponse handleHandshakeUserInfo(HttpRequest httpRequest, HttpResponse httpResponse, ChannelContext channelContext) throws  Exception {
        ...
        if (userId == null) {
            ...
            //解析token
            LayimContextUserInfo userInfo = imService.getContextUserInfo(uid);
            if (userInfo == null) {
                //没有找到用户
    ...
    private void notify(ChannelContext channelContext, boolean online) throws IOException{
        ...
        //获取用户所有的好友ID
        List<String> allFriendIds = getImService().getAllFriends(uid);
